...
export function FileIcon({ extension, className = 'w-12 h-12' }: FileIconProps) {
  const ext = extension.toLowerCase().replace('.', '')
  const config = iconMap[ext] || defaultConfig
...
export function getFileTypeColor(extension: string): string {
  const ext = extension.toLowerCase().replace('.', '')
  return iconMap[ext]?.color || defaultConfig.color
}
